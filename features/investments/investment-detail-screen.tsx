            ...
            {contributions.map((entry) => {
              ...
              return (
                <div className="detail-payout-row" key={entry.id}>
                  ...
                  <span className="payout-figure payout-figure-main"><small>Due</small><b>{formatMoney(entry.amountPaise)}</b></span>
                  <span className="payout-figure payout-figure-under-main"><small>{settled ? "Paid" : "Status"}</small><b>{settled ? formatMoney(entry.paidAmountPaise ?? entry.amountPaise) : labelType(entry.status)}</b></span>
                  {settled
...
function PayoutList({ investment, onConfirm, limit }: { investment: PortfolioInvestment; onConfirm: (payout: PayoutProjection, outcome: "received" | "not-received") => void; limit?: number }) {
  ...
  return <div className="detail-payout-list">{rows.map((payout) => {
    ...
    return <div className="detail-payout-row" key={payout.id}>
      ...
      <span className="payout-figure payout-figure-main"><small>Interest</small><b>{formatMoney(payout.grossInterestPaise)}</b>{payout.principalRepaidPaise > 0n && <small>+{formatMoney(payout.principalRepaidPaise)} principal</small>}</span>
      <span className="payout-figure payout-figure-under-date"><small>Expected TDS</small><b>{formatMoney(payout.expectedTdsPaise)}</b></span>
      <span className="payout-figure payout-figure-under-main"><small>{settled ? "Received" : "Expected credit"}</small><b>{formatMoney(payout.receivedAmountPaise ?? payout.expectedNetPaise)}</b></span>
